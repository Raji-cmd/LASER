...
scores = np.column_stack([sr, -mmr])
pareto_mask = identify_pareto(scores)
pareto_points = np.column_stack([sr[pareto_mask], mmr[pareto_mask]])
pareto_points = pareto_points[pareto_points[:, 1].argsort()]

# ================= Key Solutions =================
best_quality_idx = np.argmin(sr)
best_productivity_idx = np.argmax(mmr)
balanced_idx = np.argmin(np.sqrt((sr - sr.min())**2 + (mmr - mmr.max())**2))

# ================= Plot =================
plt.figure(figsize=(10, 8))
plt.scatter(sr, mmr, color='gray', alpha=0.6, s=50, label='Experimental Points')
...
key_points = [
    (best_quality_idx, 'Best Quality (Min SR)', 's', 'red'),
    (best_productivity_idx, 'Best Productivity (Max MMR)', '^', 'green'),
...
print(f"Best Quality → SR={sr[best_quality_idx]:.2f}, MMR={mmr[best_quality_idx]:.2f}")
print(f"Best Productivity → SR={sr[best_productivity_idx]:.2f}, MMR={mmr[best_productivity_idx]:.2f}")
print(f"Balanced → SR={sr[balanced_idx]:.2f}, MMR={mmr[balanced_idx]:.2f}")
...
summary_table = pd.DataFrame({
    'Parameter': [
        ...
        'Material melting Rate (mm³/min)',
        'Heat Affected Zone (μm)'
    ],
    'Best Quality (Min SR)': best_quality.values,
    'Best Productivity (Max MMR)': best_productivity.values,
