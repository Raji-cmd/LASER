...
# ============ Dataset ============
df = pd.DataFrame({
    'Surface Waviness': [6.07, 4.01, 6.75, 5.10, 6.01, 5.90, 6.05, 5.66,
    4.13, 5.23, 6.26, 7.32, 4.90, 5.60, 6.44, 6.02,
    4.33, 5.94, 6.21, 7.10, 4.69, 6.63, 6.43, 4.69],
    'MVR': [100.71, 74.77, 119.88, 79.11, 93.27, 99.84, 117.20, 95.14,
...
# ============ Metrices ============
metrics = {
    'Surface Waviness': {'RMSE': 0.0668, 'MAE': 0.0561, 'R2': 0.9944},
    'MVR': {'RMSE': 1.3266, 'MAE': 1.0954, 'R2': 0.9925},
    'HAZ': {'RMSE': 0.5858, 'MAE': 0.5000, 'R2': 0.9956}
}
...
# ============ Plot 1: Surface Waviness ============
fig, ax = plt.subplots(figsize=(8, 6))
plot_with_metrics(ax,
                  df['Surface Waviness'].values,
                  y_pred['Surface Waviness'].values,
                  'Surface Wavinesss Prediction',
                  'Measured Surface Waviness',
                  'Predicted Surface Waviness',
                  metrics['Surface Waviness'],
                  color='blue')
plt.tight_layout()
plt.show()
# ============ Plot 2: MVR & HAZ ============
...
targets = ['MVR', 'HAZ']
titles = ['MVR', 'HAZ']
xlabels = ['MVR', 'Measured HAZ']
ylabels = ['Predicted MVR', 'Predicted HAZ']
colors = ['green', 'purple']
