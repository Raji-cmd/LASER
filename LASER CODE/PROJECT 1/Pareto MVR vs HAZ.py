...
scores = np.column_stack([haz, -mvr])
pareto_mask = identify_pareto(scores)
pareto_points = np.column_stack([haz[pareto_mask], mvr[pareto_mask]])
pareto_points = pareto_points[pareto_points[:, 1].argsort()]

# ================= Key Solutions =================
best_quality_idx = np.argmin(haz)
best_productivity_idx = np.argmax(mvr)
balanced_idx = np.argmin(np.sqrt((haz - haz.min())**2 + (mvr - mvr.max())**2))

# ================= Plot =================
plt.figure(figsize=(10, 8))

plt.scatter(haz, mvr, color='gray', alpha=0.6, s=50, label='Experimental Points')
...
key_points = [
    (haz[best_quality_idx], mvr[best_quality_idx], "Min HAZ", 's', 'red'),
    (haz[best_productivity_idx], mvr[best_productivity_idx], "Max MVR", '^', 'green'),
    (haz[balanced_idx], mvr[balanced_idx], "Balanced", 'D', 'purple')
]
...
summary_table = pd.DataFrame({
    ...
    'Min HAZ': [
        ...
        best_quality['Material Vaporization Rate']
    ],
    'Max MVR': [
        ...
        best_productivity['Material Vaporization Rate']
    ],
    'Balanced': [
        ...
        balanced['Material Vaporization Rate']
    ]
})
